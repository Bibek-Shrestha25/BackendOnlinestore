...
exports.register = async (req, res) => {
  ...
  if (user) {
    return res
      .status(400)
      .json({ error: "Username already taken, choose another username" });
  }
  ...
  if (user) {
    return res
      .status(400)
      .json({ error: "Email already in use. Please login to continue" });
  }
  ...
  if (!address) {
    return res
      .status(400)
      .json({ error: "Something to wrong, please try again later" });
  }
...
exports.signin = async (req, res) => {
  ...
  //generate token
  let token = jwt.sign({ username, email, role, id }, process.env.JWT_SECRET, {
    expiresIn: 86400,
  });
